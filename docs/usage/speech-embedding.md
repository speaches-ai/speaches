...
# Downloading a model
uvx speaches-cli model download pyannote/wespeaker-voxceleb-resnet34-LM

# Check that the model has been installed
uvx speaches-cli model ls --task speaker-embedding | jq '.data | map(select(.id == "pyannote/wespeaker-voxceleb-resnet34-LM"))'
```
...
=== "httpx"
    ...
    with open('audio.wav', 'rb') as f:
        ...
        data = {'model': 'pyannote/wespeaker-voxceleb-resnet34-LM'}
        response = httpx.post(
